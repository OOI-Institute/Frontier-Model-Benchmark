...
def _run_text_task(adapter,task):
    ...
    for n in range(1,task.max_attempts+1):
        ...
        try:
            out=adapter.generate(prompt); latency=time.perf_counter()-t0; g=grade_output(task.grader,out)
        ...
        attempts.append(Attempt(n,out,latency,g.passed,g.partial,g.gradeable,g.violation,g.verified,g.confidence,g.failure_code,g.detail,u["action_count"],u["input_tokens"],u["output_tokens"],u["cost_usd"]))
...
def _run_environment_task(adapter,task,manifest,trial_seed):
    ...
    attempt=Attempt(1,json.dumps(state,sort_keys=True),episode.latency_s,episode.success,1.0 if episode.success else 0.0,True,episode.violation,verified,None,failure_code,detail,episode.actions,episode.input_tokens,episode.output_tokens,episode.cost_usd)
...
def run_suite(adapter,tasks,manifest:SystemManifest,out_dir="runs",trials:int=1):
    """Run one AFB experiment. Trials are independent rollouts; retries stay inside a rollout."""
    ...
    payload={"manifest":asdict(manifest)|{"benchmark":"AnyModel Frontier Benchmark","benchmark_version":"1.3.0","started_utc":started.isoformat(),"task_count":len(tasks),"trial_count":trials,"rollout_count":len(trial_results)},"metrics":metrics,"results":[r.to_dict() for r in trial_results]}
...
def summarize(trial_results,tasks,manifest,trials):
    ...
    # AFB will not publish an aggregate unless independent-trial consistency and actual
    # recovery evidence are both present. Missing dimensions stay N/A, never implicit 100s.
