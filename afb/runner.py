...
from afb.provenance import attach_fingerprint
...
def run_suite(adapter,tasks,manifest:SystemManifest,out_dir="runs",trials:int=1):
    ...
    payload={"manifest":asdict(manifest)|{"benchmark":"AnyModel Frontier Benchmark","benchmark_version":"1.3.2","started_utc":started.isoformat(),"task_count":len(tasks),"trial_count":trials,"rollout_count":len(trial_results)},"metrics":metrics,"results":[r.to_dict() for r in trial_results]}
...
def summarize(trial_results,tasks,manifest,trials):
    ...
    missing_reasons=[]
    if trials<2: missing_reasons.append("independent_trials")
    if trial_consistency is None: missing_reasons.append("trial_consistency")
    if recovery_rate is None: missing_reasons.append("recovery_evidence")
    frontier=None
