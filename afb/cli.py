...
def main():
    ...
    r.add_argument("--per-domain",type=int,default=25,help="25 = AFB Core-300; use 4 for Smoke-48")
    r.add_argument("--out-dir",default="runs")
    a=p.parse_args()

    if a.adapter=="oracle": adapter=OracleAdapter(); name="oracle-validator"
    elif a.adapter=="weak": adapter=WeakAdapter(); name="weak-control"
    ...
    manifest=SystemManifest(system_name=name,base_model=a.model or "n/a")
