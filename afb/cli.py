...
def _parse_tool_versions(items):
    ...
    for item in items or []:
        if "=" not in item:
            raise ValueError("--tool-version must use NAME=VERSION")
        k, v = item.split("=", 1)
        out[k.strip()] = v.strip()
    return out
...
def _build_adapter(a, p):
    ...
    return OpenAICompatibleAdapter(
        a.base_url, a.model, a.api_key, timeout_s=a.timeout_s,
        temperature=a.temperature, top_p=a.top_p,
        max_output_tokens=a.max_output_tokens, seed=a.inference_seed,
    ), a.system_name
...
def _add_manifest_args(parser):
    ...
    parser.add_argument("--system-prompt-hash", default="")
    parser.add_argument("--tools", default="", help="Comma-separated declared tools")
    parser.add_argument("--tool-version", action="append", default=[], help="NAME=VERSION; repeat as needed")
    parser.add_argument("--external-memory", action="store_true")
    parser.add_argument("--scaffold", default="none")
    parser.add_argument("--scaffold-version", default="unknown")
    parser.add_argument("--harness-commit-sha", default="")
    parser.add_argument("--reasoning-budget", default="standard")
    parser.add_argument("--retry-policy", default="task_defined")
    parser.add_argument("--network-policy", default="none")
    parser.add_argument("--context-policy", default="provider_default")
    parser.add_argument("--max-runtime-s", type=float)
    parser.add_argument("--max-actions", type=int)
    parser.add_argument("--max-total-tokens", type=int)
    parser.add_argument("--max-calls", type=int)
    parser.add_argument("--max-cost-usd", type=float)
    parser.add_argument("--component-label", default="full_system")
    parser.add_argument("--parent-configuration")
...
def _manifest(a, name, pack):
    ...
    return SystemManifest(
        ...
        tools=tools,
        tool_versions=_parse_tool_versions(a.tool_version),
        external_memory=a.external_memory,
        scaffold=a.scaffold,
        scaffold_version=a.scaffold_version,
        harness_commit_sha=a.harness_commit_sha,
        reasoning_budget=a.reasoning_budget,
        retry_policy=a.retry_policy,
        temperature=a.temperature,
        top_p=a.top_p,
        inference_seed=a.inference_seed,
        ...
        parent_configuration=a.parent_configuration,
    )
...
def _validate_official(manifest, trials, p):
    ...
    if trials < 2:
        missing.append("trials>=2")
    if manifest.pack == "frontier" and not manifest.tools:
        missing.append("tools")
    if missing:
        p.error("official AFB output refused; incomplete manifest: " + ", ".join(sorted(set(missing))))
...
def main():
    ...
    r = sp.add_parser("run", help="Run an AFB experiment")
    _add_adapter_args(r)
    _add_manifest_args(r)
    r.add_argument("--pack", choices=["smoke", "diagnostic", "frontier"], default="diagnostic")
    r.add_argument("--seed", type=int, default=20260903)
    r.add_argument("--trials", type=int, default=1, help="Independent rollouts per task; distinct from retries")
    r.add_argument("--per-domain", type=int, help="Override pack default instances/domain")
    r.add_argument("--out-dir", default="runs")
    r.add_argument("--human-baselines", help="JSON file of measured task timings keyed by task_id")
    ...
    if a.cmd == "import-terminal":
        result = normalize_terminal_results(a.input)
    ...
    if a.pack == "frontier":
        per_domain = a.per_domain if a.per_domain is not None else 2
        tasks = generate_frontier_suite(a.seed, per_domain)
    else:
        per_domain = a.per_domain if a.per_domain is not None else (4 if a.pack == "smoke" else 25)
        tasks = generate_reference_suite(a.seed, per_domain)

    if a.human_baselines:
        apply_human_baselines(tasks, load_human_baselines(a.human_baselines))

    manifest = _manifest(a, name, a.pack)
    _validate_official(manifest, a.trials, p)
