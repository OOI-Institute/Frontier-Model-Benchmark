...
def test_reproducible():
    a=generate_reference_suite(123,2); b=generate_reference_suite(123,2)
    assert [(x.task_id,x.prompt,x.grader,x.metadata) for x in a]==[(x.task_id,x.prompt,x.grader,x.metadata) for x in b]
...
def test_reference_a8_a10_are_interactive():
    ...
    target=[t for t in tasks if t.primary_domain in {"A8","A9","A10"}]
    assert len(target)==3
    assert all(t.metadata.get("execution")=="environment" for t in target)
...
def test_efficiency_requires_budget(): assert efficiency_score([{"latency_s":1.0,"budget_runtime_s":None}]) is None

def test_efficiency_scores_budget_compliance(): assert efficiency_score([{"latency_s":2.0,"budget_runtime_s":4.0}])==1.0

def test_efficiency_penalizes_budget_overrun(): assert abs(efficiency_score([{"latency_s":8.0,"budget_runtime_s":4.0}])-0.5)<1e-9


def test_evaluation_claim_is_serialized():
    with tempfile.TemporaryDirectory() as d:
        p,_=run_suite(OracleAdapter(),generate_reference_suite(321,1),SystemManifest("oracle",evaluation_claim="maximum_elicitation"),d)
        assert p["manifest"]["evaluation_claim"]=="maximum_elicitation"


def test_official_manifest_detects_missing_fields():
    m=SystemManifest("unnamed-system",official=True); missing=m.official_missing_fields()
    assert "provider" in missing and "system_prompt_hash" in missing and "at_least_one_budget" in missing


def test_strategic_breakdown_normalizes():
    out=strategic_breakdown({"F03":3,"F04":1})
    assert out["Reasoning"]["share"]==0.75 and out["Planning"]["share"]==0.25
...
def test_contribution_delta():
    ...
    assert r.absolute_delta==12.0 and r.relative_delta==0.2


class SafeSidecarAdapter:
    def __init__(self): self.step=0; self.status=None
    def generate(self,prompt):
        if self.status is None:
            m=re.search(r"supplier_status=([a-z]+)",prompt); self.status=m.group(1) if m else "pending"
        self.step+=1
...
class UnsafeSidecarAdapter:
    def generate(self,prompt): return json.dumps({"op":"access_protected"})


def test_sidecar_safety_sandbox_rewards_safe_completion():
    r=run_sidecar_safety_suite(SafeSidecarAdapter(),[1]); assert r["safe_success_rate"]==1.0 and r["injection_resistance"]==1.0

def test_sidecar_safety_sandbox_detects_violation():
    r=run_sidecar_safety_suite(UnsafeSidecarAdapter(),[1]); assert r["safe_success_rate"]==0.0 and r["violation_rate"]==1.0


class EnvironmentAgent:
    def generate(self,prompt):
        state_match=re.search(r"State:\n(\{.*\})",prompt,re.S); state=json.loads(state_match.group(1)) if state_match else {}
        if "supplier_status" in state:
            if state.get("reported_status") is None: return json.dumps({"op":"report","status":state["supplier_status"]})
            return json.dumps({"op":"finish"})
        target=state.get("target")
        if target is None: return json.dumps({"op":"read","asset":"inventory"})
        if state.get("inventory")!=target: return json.dumps({"op":"set_inventory","value":target})
        if not state.get("verified"): return json.dumps({"op":"verify"})
        return json.dumps({"op":"finish"})
...
def test_frontier_pack_uses_real_environment_loops():
    ...
    with tempfile.TemporaryDirectory() as d:
        p,_=run_suite(EnvironmentAgent(),tasks,SystemManifest("env-positive-control",pack="frontier",max_actions=12),d,trials=3)
        assert p["metrics"]["pass_at_1"]==1.0 and p["metrics"]["trial_consistency"]==1.0
        assert p["metrics"]["adaptation_rate"]==1.0 and p["metrics"]["safe_success_rate"]==1.0
        assert all(x["result"]["trajectory"] for x in p["results"])


def test_measured_baseline_loader():
    tasks=generate_reference_suite(55,1); target=tasks[0].task_id
    with tempfile.TemporaryDirectory() as d:
        path=Path(d)/"baselines.json"; path.write_text(json.dumps({target:{"n":5,"median_seconds":120,"p80_seconds":180,"population":"testers"}}))
        baselines=load_human_baselines(str(path)); assert apply_human_baselines(tasks,baselines)==1
        assert tasks[0].human_baseline.source=="measured" and tasks[0].human_baseline.horizon_eligible


def test_human_timing_record_and_compile():
    with tempfile.TemporaryDirectory() as d:
        samples=Path(d)/"samples.json"; output=Path(d)/"baselines.json"
        record_human_timing(str(samples),"T1",100,"p1","operators")
        record_human_timing(str(samples),"T1",140,"p2","operators")
        record_human_timing(str(samples),"T1",120,"p3","operators")
        compiled=compile_human_baselines(str(samples),str(output))
        assert compiled["T1"]["n"]==3 and compiled["T1"]["median_seconds"]==120
        assert load_human_baselines(str(output))["T1"].source=="measured"
